...
#from Delight_pvt.activation_layers import get_activation_layer
import math
import numpy as np
from typing import Optional
# from Delight_pvt.print_utilities import *
...
if __name__ == '__main__':
    ...
    feature = torch.rand(8, 256, 14, 20)        # [H*W, bh, channel]
    feature = feature.flatten(2).permute(1, 0, 2)
    
    def __init__(self, embed_dim, width_multiplier=2.0, dextra_depth=4, dextra_proj=2, heads=8, sr_ratio=2):
        super().__init__()
        self.embed_dim = embed_dim        #  channel
        self.heads = heads
    ...
    sr_ratio = 2
    dextra_proj = 2
    proj_dim = embed_dim // dextra_proj
        
    dextra_layer = DExTraUnit(in_features = embed_dim,
                                       in_proj_features = proj_dim,
    ...
    feature_af = dextra_layer(feature)    


    num_para = 0
    for name, param in dextra_layer.named_parameters():
        num_mul = 1
        for x in param.size():
            num_mul *= x
        num_para += num_mul
    print("Number of trainable parameters {0} in Model {1}".format(num_para, "dextra_layer"))
    print("------------------------------------------")
        
    print("feature_af.shape is: ", feature_af.shape) 
