...
class U_ResTran3D(nn.Module):
    def __init__(self, norm_cfg='BN', activation_cfg='ReLU', img_size=None, num_classes=None, weight_std=False):
        ...
        self.upsamplex444 = nn.Upsample(scale_factor=(4,4,4), mode='trilinear')

        self.transposeconv_stage2 = nn.ConvTranspose3d(self.dims[-1], self.dims[-1], kernel_size=(2,2,2), stride=(2,2,2), bias=False)
        self.transposeconv_stage1 = nn.ConvTranspose3d(self.dims[-1], self.dims[0], kernel_size=(2,2,2), stride=(2,2,2), bias=False)
        self.transposeconv_stage0 = nn.ConvTranspose3d(self.dims[0], 64, kernel_size=(2,2,2), stride=(2,2,2), bias=False)

        self.stage2_de = ResBlock(self.dims[-1], self.dims[-1], norm_cfg, activation_cfg, weight_std=weight_std)
        self.stage1_de = ResBlock(self.dims[0], self.dims[0], norm_cfg, activation_cfg, weight_std=weight_std)
        self.stage0_de = ResBlock(64, 64, norm_cfg, activation_cfg, weight_std=weight_std)

        self.ds2_cls_conv = nn.Conv3d(self.dims[-1], self.MODEL_NUM_CLASSES, kernel_size=1)  # num_classes 14
        self.ds1_cls_conv = nn.Conv3d(self.dims[0], self.MODEL_NUM_CLASSES, kernel_size=1)
        self.ds0_cls_conv = nn.Conv3d(64, self.MODEL_NUM_CLASSES, kernel_size=1)
        
        self.conv_s0 = nn.Conv3d(self.dims[-1], self.dims[0], kernel_size=(1,1,1), bias=False)
        self.conv_s1 = nn.Conv3d(self.dims[0], self.dims[-1], kernel_size=(2,2,2), stride=(2,2,2), bias=False)
        self.conv_s2 = nn.Conv3d(self.dims[-1], self.dims[-1], kernel_size=(2,2,2), stride=(2,2,2), bias=False)
        ...
        self.backbone = CNNBackbone_rd.Backbone(depth=9, norm_cfg=norm_cfg, weight_std=weight_std, dims = [self.dims[0], self.dims[-1], self.dims[-1]]) 
        total = sum([param.nelement() for param in self.backbone.parameters()])
        print('  + Number of Backbone Params: %.2f(e6)' % (total / 1e6))

        self.position_embed_1 = build_position_encoding(mode='v2', hidden_dim=self.dims[0])
        self.position_embed_23 = build_position_encoding(mode='v2', hidden_dim=self.dims[-1])
        self.Delight_Trans_lv1 = effTrans_layers(self.dims[0], heads=8, sr_ratio=2)   
        self.Delight_Trans_lv2 = effTrans_layers(self.dims[-1], heads=8, sr_ratio=2)   
        self.Delight_Trans_lv3 = effTrans_layers(self.dims[-1], heads=8, sr_ratio=2)   
        total_1 = sum([param.nelement() for param in self.Delight_Trans_lv1.parameters()])
        total_2 = sum([param.nelement() for param in self.Delight_Trans_lv2.parameters()])
        total_3 = sum([param.nelement() for param in self.Delight_Trans_lv3.parameters()])
        print('  + Number of Delight_sra_lv1 Params: %.2f(e6)' % (total_1 / 1e6))
        print('  + Number of Delight_sra_lv2 Params: %.2f(e6)' % (total_2 / 1e6))
        print('  + Number of Delight_sra_lv3 Params: %.2f(e6)' % (total_3 / 1e6))
    ...
    def unify_shape(self, x_trans_lv1, x_trans_lv2, x_trans_lv3):
    
        # untify to level 3, and fusion them
        x_rs_lv1 = x_trans_lv1.permute(1, 2, 0).view(-1, self.dims[0], 24, 48, 48)
        x_rs_lv2 = x_trans_lv2.permute(1, 2, 0).view(-1, self.dims[-1], 12, 24, 24)
        x_rs_lv3 = x_trans_lv3.permute(1, 2, 0).view(-1, self.dims[-1], 6, 12, 12)         
        
        x_lv1_3 = self.conv_s2(self.conv_s1(x_rs_lv1)) 
        x_lv2_3 = self.conv_s2(x_rs_lv2)               
        x_3_fused = x_lv1_3 + x_lv2_3 + x_rs_lv3       

        # untify to level 2, and fusion them
        x_lv1_2 = self.conv_s1(x_rs_lv1)               
        x_lv3_2 = self.upsamplex222(x_rs_lv3)          
        x_2_fused = x_lv1_2 + x_lv3_2 + x_rs_lv2       
        
        # untify to level 1, and fusion them
        x_lv2_1 = self.upsamplex222(self.conv_s0(x_rs_lv2))  
        x_lv3_1 = self.upsamplex444(self.conv_s0(x_rs_lv3))  
        x_1_fused = x_lv2_1 + x_lv3_1 + x_rs_lv1             
        
        
        return x_1_fused, x_2_fused, x_3_fused                         
    ...
    def forward(self, inputs):
        ...
        # level 1 delight transformer first time
        x_lv1 = x_fea[0] + x_posemb[0]             
        x_lv1 = x_lv1.flatten(2).permute(2, 0, 1)  
        lv1_mask = masks[1].flatten(1)               
        y_lv1 = self.Delight_Trans_lv1(x_lv1, lv1_mask)  
       
       # level 2 delight transformer first time
        x_lv2 = x_fea[1] + x_posemb[1]             
        x_lv2 = x_lv2.flatten(2).permute(2, 0, 1)  
        lv2_mask = masks[1].flatten(1)               
        y_lv2 = self.Delight_Trans_lv2(x_lv2, lv2_mask)  
       
       # level 3 delight transformer first time
        x_lv3 = x_fea[2] + x_posemb[2]             
        x_lv3 = x_lv3.flatten(2).permute(2, 0, 1)  
        lv3_mask = masks[2].flatten(1)               
        y_lv3 = self.Delight_Trans_lv3(x_lv3, lv3_mask)  
        
        # fuse first result
        x_lv1_fused, x_lv2_fused, x_lv3_fused = self.unify_shape(y_lv1, y_lv2, y_lv3)  

        # level 1 delight transformer second time
        x_lv1_fused = x_lv1_fused.flatten(2).permute(2, 0, 1)  
        y_lv1_fused = self.Delight_Trans_lv1(x_lv1_fused, lv1_mask)   

        # level 2 delight transformer second time
        x_lv2_fused = x_lv2_fused.flatten(2).permute(2, 0, 1)  
        y_lv2_fused = self.Delight_Trans_lv2(x_lv2_fused, lv2_mask)       
               
        # level 3 delight transformer second time
        x_lv3_fused = x_lv3_fused.flatten(2).permute(2, 0, 1)  
        y_lv3_fused = self.Delight_Trans_lv3(x_lv3_fused, lv3_mask)  
        
        # fuse SECOND result
        x_lv1_fused_2, x_lv2_fused_2, x_lv3_fused_2 = self.unify_shape(y_lv1_fused, y_lv2_fused, y_lv3_fused) 

        # level 1 delight transformer third time
        x_lv1_fused_2 = x_lv1_fused_2.flatten(2).permute(2, 0, 1)  
        y_lv1_fused_2 = self.Delight_Trans_lv1(x_lv1_fused_2, lv1_mask)  
        
        # level 2 delight transformer third time
        x_lv2_fused_2 = x_lv2_fused_2.flatten(2).permute(2, 0, 1)  
        y_lv2_fused_2 = self.Delight_Trans_lv2(x_lv2_fused_2, lv2_mask)  
       
        # level 3 delight transformer third time
        x_lv3_fused_2 = x_lv3_fused_2.flatten(2).permute(2, 0, 1)  
        y_lv3_fused_2 = self.Delight_Trans_lv3(x_lv3_fused_2, lv3_mask)  
        ...
        skip3 = y_lv1_fused_2.permute(1, 2, 0).view(x_convs[-3].shape) 
...
class TransHRNet_v1(SegmentationNetwork):
    """
    TransHRNet-3D
    """
    ...
    def forward(self, x):
        seg_output = self.U_ResTran3D(x)        # to 143 lines
