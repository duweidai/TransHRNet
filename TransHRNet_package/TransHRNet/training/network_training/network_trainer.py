...
class NetworkTrainer(object):
        ...
        while self.epoch < 1010:            
            ...
            if self.use_progress_bar:
                with trange(self.num_batches_per_epoch) as tbar:
                    for b in tbar:
                        tbar.set_description("Epoch {}/{}".format(self.epoch+1, 1010))   # self.max_num_epochs
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        if torch.cuda.is_available():
            ...
            weight_numpy_path = os.path.join(self.output_folder, "img_and_npy")
            if not os.path.exists(weight_numpy_path): 
                os.makedirs(weight_numpy_path)
            target_npy = target.cpu().detach().numpy().astype(np.float)
            np.save(os.path.join(weight_numpy_path, "label_{}.npy".format(1)), target_npy)
